...
const STATUS_BADGE: Record<
  string,
  { bg: string; text: string; label: Record<Locale, string> }
> = {
  ACTIVE: {
    bg: Colors.primary,
    text: '#fff',
...
export function VehicleCard({ vehicle, locale, onPress }: Props) {
  ...
  const profilePhoto =
    vehicle.photos?.find((p) => p.isProfile) ?? vehicle.photos?.[0];
  const photoUrl = resolvePhotoUrl(profilePhoto?.url);
  const cardDirectionStyle = !rtl ? styles.rowReverse : undefined;
  const plateRowDirectionStyle = !rtl ? styles.rowReverse : undefined;
  const subtitleAlignStyle = rtl ? styles.textLeft : styles.textRight;
    ...
    >
      <View style={styles.iconWrap}>
        ...
        {photoUrl ? (
          ...
          <Text style={styles.plateAbbr} numberOfLines={1} adjustsFontSizeToFit>
            {plateAbbr}
          </Text>
        )}
      </View>
      <View style={styles.body}>
        <View style={[styles.plateRow, plateRowDirectionStyle]}>
