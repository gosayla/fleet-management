...
function Wheel({
    ...
    setTimeout(
      () =>
        ref.current?.scrollTo({
          y: Math.max(0, initialIndex) * WHEEL_H,
          animated: false,
        }),
      80
    );
  ...
  function snap(e: any) {
    ...
    const idx = Math.max(
      0,
      Math.min(Math.round(raw / WHEEL_H), items.length - 1)
    );
  ...
  const widthStyle = width == null ? undefined : { width, flex: 0 as const };

  return (
    <View style={[styles.col, widthStyle]}>
...
export function DateTimeWheelModal({
  ...
  const headerDirectionStyle = isRTL ? styles.headerRtl : undefined;
  const wheelsDirectionStyle = isRTL ? styles.wheelsRtl : undefined;
  ...
  return (
    ...
    >
      ...
      <View style={styles.sheet}>
        ...
        <View style={[styles.wheels, wheelsDirectionStyle]}>
          <Wheel
            key={`d-${wheelResetKey}`}
...
const styles = StyleSheet.create({
  ...
  wheelsRtl: { flexDirection: 'row-reverse' },
  col: { flex: 1, position: 'relative' },
  wheelContent: { paddingVertical: WHEEL_H * Math.floor(VISIBLE / 2) },
  wheelScroll: { height: WHEEL_H * VISIBLE },
  highlight: {
    position: 'absolute',
    left: 0,
    right: 0,
