    ...
    api
      ...
      .then((tripData) => {
        setForm({
          ...
          tripType: (tripData.tripType as TripType) ?? 'ONE_TIME',
          scheduledStart: tripData.scheduledStart
            ? toLocalInput(tripData.scheduledStart)
            : '',
          scheduledEnd: tripData.scheduledEnd
            ? toLocalInput(tripData.scheduledEnd)
            : '',
          ...
          status: (tripData.status as TripStatus) ?? 'SCHEDULED',
        });
      })
      .catch(() => setError(i18n.failedToLoadTrip))
      .finally(() => setLoadingTrip(false));
  }, [i18n.failedToLoadTrip, tripId]);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      try {
        ...
        if (!shouldRetryWithoutName(e, payload)) {
          throw e;
        }

        const fallbackPayload = (({ name: _ignoredName, ...rest }) => rest)(
          payload
        );
        if (isEdit) {
          await api.patch(`/trips/${tripId}`, {
        ...
        Alert.alert(i18n.saved, i18n.tripSavedNameMsg);
      }
  ...
  return (
    <View style={styles.container}>
        ...
        >
          ...
          <View style={styles.card}>
            ...
            <Field label={i18n.tripTypeField}>
              <View style={[styles.pillsRow, rowDirectionStyle]}>
                {TRIP_TYPES.map((tt) => (
                  <TouchableOpacity
                    key={tt}
                    style={[
                      styles.pill,
                      form.tripType === tt && styles.pillActive,
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        styles.pillText,
                        form.tripType === tt && styles.pillTextActive,
                      ]}
                    >
            ...
            <Field label={i18n.startDateTimeLabel} required>
              ...
              >
                ...
                <Text
                  style={[
                    styles.dateBtnText,
                    !form.scheduledStart && styles.dateBtnPlaceholder,
                  ]}
                >
            ...
            <Field label={i18n.endDateTimeLabel} required>
              ...
              >
                ...
                <Text
                  style={[
                    styles.dateBtnText,
                    !form.scheduledEnd && styles.dateBtnPlaceholder,
                  ]}
                >
          ...
          <TouchableOpacity
            style={[styles.pickerBtn, rowDirectionStyle]}
            ...
            {selectedDriver ? (
              <View style={[styles.pickerSelected, rowDirectionStyle]}>
                <View style={styles.pickerAvatar}>
                  <Text style={[styles.pickerAvatarText, textStartStyle]}>
                ...
                <View style={styles.flexOne}>
                  <Text style={[styles.pickerName, textEndStyle]}>
                    {selectedDriver.fullName}
                  </Text>
                  <Text style={[styles.pickerSub, textEndStyle]}>
                    {selectedDriver.phone}
                  </Text>
                </View>
              </View>
            ) : (
              <View style={[styles.pickerPlaceholder, rowDirectionStyle]}>
                <AppIcon
          ...
          >
            {selectedVehicle ? (
              <View style={[styles.pickerSelected, rowDirectionStyle]}>
                <View style={[styles.pickerAvatar, styles.pickerVehicleAvatar]}>
                  ...
                  return (
                    <React.Fragment key={s}>
                      ...
                      >
                        ...
                        >
                          {TRIP_STATUS_LABELS[s][locale] ??
                            TRIP_STATUS_LABELS[s].en}
                        </Text>
                        {isActive && (
                          <AppIcon
                            name="check-circle"
      ...
      <DateTimeWheelModal
        ...
        label={
          dtPicker === 'scheduledStart'
            ? i18n.startDateTimeLabel
            : i18n.endDateTimeLabel
        }
...
function shouldRetryWithoutName(
  error: any,
  ...
  return (
    text.includes('name') &&
    (text.includes('non-whitelisted') || text.includes('غير مسموح بالحقل'))
  );
}
              ...
              return (
                ...
                >
                  {item.initials ? (
                    ...
                    <View
                      style={[styles.modalAvatar, styles.pickerVehicleAvatar]}
                    >
                      <AppIcon name="truck" size={18} color={Colors.primary} />
                    </View>
                  )}
                  <View style={styles.flexOne}>
                    <Text
                      style={[
                        styles.modalItemPrimary,
                        textEndStyle,
                        isSelected && styles.modalItemPrimarySelected,
                      ]}
                    >
                      {item.primary}
                    </Text>
                    <Text style={[styles.modalItemSecondary, textEndStyle]}>
                      {item.secondary}
                    </Text>
                  </View>
                  {isSelected && (
...
const styles = StyleSheet.create({
  ...
  pickerVehicleAvatar: { backgroundColor: '#e8f5f4' },
  pickerName: {
    fontSize: 15,
    fontWeight: '600' as const,
    color: Colors.textPrimary,
  ...
  modalItemPrimarySelected: { color: Colors.primary },
  modalAvatar: {
    width: 40,
    height: 40,
    borderRadius: 20,
