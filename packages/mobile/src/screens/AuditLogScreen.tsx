...
function describeLog(item: AuditLog): string {
  ...
  switch (item.action) {
    ...
    default:
      return `${actor} performed a ${entity} action`;
  }
}
...
export function AuditLogScreen({ locale, onBack }: Props) {
  ...
  const fetchPage = useCallback(
    async (p: number, f: ActionFilter, append = false) => {
    ...
    return (
      ...
      >
        ...
        <View style={styles.cardBody}>
          ...
          <Text
            style={[styles.description, { textAlign: !rtl ? 'right' : 'left' }]}
            numberOfLines={2}
          >
            {description}
          </Text>
