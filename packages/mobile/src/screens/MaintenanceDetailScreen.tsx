...
export function MaintenanceDetailScreen({maintenanceId, locale, onBack, onEdit, onDeleted}: Props) {
  ...
  async function load() {
    try {
      const data = await api.get<MaintenanceLog>(`/maintenance/${maintenanceId}`);
      setLog(data);
    } catch {}
    setLoading(false);
  }
  ...
  async function cancelMaintenance() {
    Alert.alert(i18n.cancelMaintenance, i18n.cancelMaintenanceConfirm, [
      {text: i18n.cancel, style: 'cancel'},
      {text: i18n.confirm, style: 'destructive', onPress: async () => {
        setActionLoading(true);
        try {
  ...
  async function completeMaintenance() {
    setCompleting(true);
    try {
      const payload: any = {status: 'COMPLETED', completedDate: new Date().toISOString()};
      if (costInput.trim()) payload.costSar = parseFloat(costInput);
      if (odometerInput.trim()) payload.odometerAtService = parseFloat(odometerInput);
  ...
  async function deleteMaintenance() {
    Alert.alert(i18n.deleteLabel, i18n.deleteMaintenanceConfirm, [
      {text: i18n.cancel, style: 'cancel'},
      {text: i18n.deleteLabel, style: 'destructive', onPress: async () => {
        try {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerRow}>
          ...
          <View style={styles.headerRight}>
            ...
            <TouchableOpacity onPress={deleteMaintenance} style={styles.iconBtn}>
      ...
      <ScrollView
        ...
        <View style={[styles.statusBanner, {backgroundColor: sc.bg}]}>
          <Text style={[styles.statusBannerText, {color: sc.color}]}>{statusLabel(log.status)}</Text>
        </View>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.vehicleRow}>
            ...
            <View style={{marginStart: 10, flex: 1}}>
              <Text style={styles.plateText}>{log.vehicle?.plateNumber ?? '—'}</Text>
              {log.vehicle && (
                <Text style={styles.subText}>{log.vehicle.make} {log.vehicle.model}</Text>
              )}
            </View>
          </View>
        </View>
        ...
        <View style={styles.card}>
          ...
          <Divider />
          <Row label={i18n.scheduledDateLabel} value={fmtDate(log.scheduledDate)} />
          {log.completedDate && <>
            <Divider />
            <Row label={i18n.completedDateLabel} value={fmtDate(log.completedDate)} />
          </>}
          {log.costSar != null && <>
            <Divider />
            <Row label={i18n.costSarLabel} value={`${log.costSar} ${i18n.sarUnit}`} />
          </>}
          {log.odometerAtService != null && <>
            <Divider />
            <Row label={i18n.odometerAtServiceLabel} value={`${log.odometerAtService} ${i18n.kmUnit}`} />
          </>}
        </View>
        ...
        {(log.nextServiceKm != null || log.nextServiceDate) && (
          <View style={styles.card}>
            ...
            {log.nextServiceKm != null && <>
              <Row label={i18n.nextServiceKmLabel} value={`${log.nextServiceKm} ${i18n.kmUnit}`} />
            </>}
            {log.nextServiceDate && <>
              {log.nextServiceKm != null && <Divider />}
              <Row label={i18n.nextServiceDateLabel} value={fmtDate(log.nextServiceDate)} />
            </>}
          </View>
        )}

        {/* Actions */}
        {actionLoading && <ActivityIndicator color={Colors.primary} style={{marginVertical: 8}} />}
        {!actionLoading && (
          <View style={styles.actionsRow}>
            {canStart && (
              <TouchableOpacity style={[styles.actionBtn, {backgroundColor: '#1565C0'}]} onPress={startWork}>
                <AppIcon name="wrench" size={18} color="#fff" />
                <Text style={styles.actionBtnText}>{i18n.maintenanceStartWork}</Text>
              </TouchableOpacity>
            )}
            {canComplete && (
              <TouchableOpacity style={[styles.actionBtn, {backgroundColor: Colors.primary}]} onPress={() => setCompleteModalOpen(true)}>
                <AppIcon name="check-circle-outline" size={18} color="#fff" />
                <Text style={styles.actionBtnText}>{i18n.maintenanceComplete}</Text>
              </TouchableOpacity>
            )}
            {canCancel && (
              <TouchableOpacity style={[styles.actionBtn, {backgroundColor: Colors.danger}]} onPress={cancelMaintenance}>
                <AppIcon name="close-circle-outline" size={18} color="#fff" />
                <Text style={styles.actionBtnText}>{i18n.cancelMaintenance}</Text>
              </TouchableOpacity>
            )}
          </View>
        )}
      ...
      {/* Complete modal */}
      <Modal visible={completeModalOpen} transparent animationType="slide" onRequestClose={() => setCompleteModalOpen(false)}>
...
const styles = StyleSheet.create({
  ...
  statusBanner: {borderRadius: 12, padding: 14, marginBottom: 14, alignItems: 'center'},
  ...
  cardLabel: {fontSize: 12, color: Colors.textMuted, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 12},
  ...
  rowValue: {fontSize: 14, color: Colors.textPrimary, fontWeight: '600', flex: 1, textAlign: 'right'},
  ...
  fieldLabel: {fontSize: 13, color: Colors.textMuted, marginBottom: 6, marginTop: 12},
  input: {borderWidth: 1, borderColor: Colors.borderLight, borderRadius: 10, padding: 12, fontSize: 15, color: Colors.textPrimary},
