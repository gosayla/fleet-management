...
export function RentalFormScreen({rentalId, locale, onBack, onSuccess}: Props) {
  const i18n = t(locale);
  ...
  async function handleSubmit() {
    setError('');
    if (!form.clientName.trim()) { setError(i18n.requiredField + i18n.rentalClientName); return; }
    if (!form.rentalStart || !form.rentalEnd) { setError(i18n.requiredField + i18n.rentalStart); return; }
    ...
    const payload: Record<string, unknown> = {
      ...
      ...(form.clientNationalId.trim() && {clientNationalId: form.clientNationalId.trim()}),
      ...
      ...(form.dailyRateSar.trim() && {dailyRateSar: Number(form.dailyRateSar)}),
      ...(form.contractFileUrl.trim() && {contractFileUrl: form.contractFileUrl.trim()}),
  ...
  return (
    <KeyboardAvoidingView style={{flex: 1}} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      <ScrollView style={styles.body} contentContainerStyle={styles.bodyContent} keyboardShouldPersistTaps="handled">
        ...
        {/* Vehicle Details */}
        <SectionTitle title={locale === 'ar' ? 'تفاصيل المركبة' : 'Vehicle Details'} />
        <View style={styles.card}>
