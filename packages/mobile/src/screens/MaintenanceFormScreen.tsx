...
export function MaintenanceFormScreen({
  ...
  const [datePickerField, setDatePickerField] = useState<
    'scheduledDate' | 'nextServiceDate' | 'completedDate' | null
  >(null);
  ...
  useEffect(() => {
    ...
    api
      .get<any>(`/maintenance/${maintenanceId}`)
      .then((m) => {
        setForm({
          ...
          nextServiceDate: m.nextServiceDate
            ? m.nextServiceDate.slice(0, 10)
            : '',
          status: m.status ?? 'PENDING',
          completedDate: m.completedDate ? m.completedDate.slice(0, 10) : '',
          costSar: m.costSar != null ? String(m.costSar) : '',
          odometerAtService:
            m.odometerAtService != null ? String(m.odometerAtService) : '',
        });
      })
      .catch(() => {});
  }, [isEdit, maintenanceId]);
  ...
  async function save() {
    ...
    if (!form.description.trim()) {
      setError(i18n.maintenanceDescriptionRequired);
      return;
    }
    if (!form.scheduledDate) {
      setError(i18n.scheduledDateRequired);
      return;
    }
    ...
    try {
      ...
      if (form.nextServiceKm.trim()) {
        payload.nextServiceKm = parseFloat(form.nextServiceKm);
      }
      if (form.nextServiceDate) {
        payload.nextServiceDate = new Date(form.nextServiceDate).toISOString();
      }
      ...
      if (isEdit) {
        ...
        if (form.costSar.trim()) {
          payload.costSar = parseFloat(form.costSar);
        }
        if (form.odometerAtService.trim()) {
          payload.odometerAtService = parseFloat(form.odometerAtService);
        }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerRow}>
          ...
          <Text style={styles.headerTitle}>
            {isEdit ? i18n.editMaintenance : i18n.addMaintenance}
          </Text>
          <TouchableOpacity
            onPress={save}
      ...
      >
          ...
          {MAINTENANCE_TYPES.map((tp) => (
            ...
            >
              <Text
                style={[
                  styles.typeBtnText,
                  form.type === tp && styles.typeBtnTextActive,
                ]}
              >
                {typeLabel(tp)}
              </Text>
        ...
        <TouchableOpacity
          ...
          onPress={() => setDatePickerField('scheduledDate')}
        >
          <Text
            style={
              form.scheduledDate ? styles.dateValue : styles.datePlaceholder
            }
          >
        ...
        <TouchableOpacity
          style={styles.dateRow}
          onPress={() => setDatePickerField('nextServiceDate')}
        >
          <Text
            style={
              form.nextServiceDate ? styles.dateValue : styles.datePlaceholder
            }
          >
        ...
        {isEdit && (
          <>
            ...
            <View style={styles.typeRow}>
              {MAINTENANCE_STATUSES.map((s) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.typeBtn,
                    form.status === s && styles.typeBtnActive,
                  ]}
                ...
                >
                  <Text
                    style={[
                      styles.typeBtnText,
                      form.status === s && styles.typeBtnTextActive,
                    ]}
                  >
                    {statusLabel(s)}
                  </Text>
            ...
            <TouchableOpacity
              style={styles.dateRow}
              onPress={() => setDatePickerField('completedDate')}
            >
              <Text
                style={
                  form.completedDate ? styles.dateValue : styles.datePlaceholder
                }
              >
          ...
          <View style={styles.pickerSheet}>
            ...
            {filteredVehicles.map((v) => (
              <TouchableOpacity
                ...
                style={[
                  styles.pickerItem,
                  form.vehicleId === v.id && styles.pickerItemActive,
                ]}
                onPress={() => {
                  set('vehicleId', v.id);
