...
export function ContractFormScreen({
  ...
  }, [contractId, i18n.failedToLoadContract, isEdit]);

  async function handleSubmit() {
    setError('');
    if (!form.clientName.trim()) {
      setError(i18n.requiredField + i18n.contractClientName);
      return;
    }
    if (!form.origin.trim() || !form.destination.trim()) {
      setError(i18n.requiredField + i18n.contractOrigin);
      return;
    }
    if (!form.contractStart || !form.contractEnd) {
      setError(i18n.requiredField + i18n.contractStart);
      return;
    }
    if (!form.departureTime.trim()) {
      setError(i18n.requiredField + i18n.departureTime);
      return;
    }
    ...
    const payload: Record<string, unknown> = {
      ...
      ...(form.isTwoWay &&
        form.returnTime.trim() && { returnTime: form.returnTime.trim() }),
  ...
  const oppositeRowDirectionStyle = isRTL ? styles.rowReverse : styles.row;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Dates & Times */}
        <SectionTitle
          title={locale === 'ar' ? 'التواريخ والأوقات' : 'Dates & Times'}
        />
        <View style={styles.card}>
          <FormField label={i18n.contractStart} required>
            <TouchableOpacity
              style={[styles.datePickerBtn, oppositeRowDirectionStyle]}
              onPress={() => setDatePickerField('contractStart')}
            >
              <Text
                style={[
                  form.contractStart
          ...
          <FormField label={i18n.contractEnd} required>
            <TouchableOpacity
              style={[styles.datePickerBtn, oppositeRowDirectionStyle]}
              onPress={() => setDatePickerField('contractEnd')}
            >
              <Text
                style={[
                  form.contractEnd
          ...
          <FormField label={i18n.departureTime} required>
            <TouchableOpacity
              style={[styles.datePickerBtn, oppositeRowDirectionStyle]}
              onPress={() => setTimePickerField('departureTime')}
            >
              <Text
                style={[
                  form.departureTime
          ...
          {form.isTwoWay && (
            <>
              ...
              <FormField label={i18n.returnTime}>
                <TouchableOpacity
                  style={[styles.datePickerBtn, oppositeRowDirectionStyle]}
                  onPress={() => setTimePickerField('returnTime')}
                >
                  <Text
                    style={[
                      form.returnTime
        ...
        <View style={styles.card}>
          <View style={[styles.switchRow, oppositeRowDirectionStyle]}>
            <Text style={[styles.switchLabel, textStartStyle]}>
              {i18n.isTwoWay}
            </Text>
            <Switch
              value={form.isTwoWay}
        ...
        <View style={styles.card}>
          ...
          >
            <Text
              style={[
                styles.pickerBtnText,
                !selectedDriver && styles.pickerPlaceholder,
                textEndStyle,
              ]}
            >
              {selectedDriver
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  style={[
                    styles.modalItem,
                    form.driverId === item.id && styles.modalItemActive,
                  ]}
                  onPress={() => {
                    set('driverId', item.id);
                ...
                >
                  <Text
                    style={[
                      styles.modalItemText,
                      form.driverId === item.id && styles.modalItemTextActive,
                      textEndStyle,
                    ]}
                  >
                    {item.fullName}
              ...
              ListEmptyComponent={
                <Text style={styles.modalEmpty}>{i18n.noDrivers}</Text>
              }
            />
          </View>
        </View>
