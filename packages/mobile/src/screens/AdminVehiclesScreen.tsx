...
export function AdminVehiclesScreen({ locale }: Props) {
  ...
  const {
    ...
    refreshing,
    refresh: load,
  } = useCachedFetch('admin:vehicles:v2', () =>
    api.get<{ data: VehicleCardData[]; total: number } | VehicleCardData[]>(
      '/vehicles?limit=500'
    )
  );
  const all: VehicleCardData[] =
    raw == null
      ? []
      : Array.isArray(raw)
      ? raw
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <VehicleCard vehicle={item} locale={locale} />
        )}
        ListEmptyComponent={
