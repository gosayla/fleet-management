...
const EXTRA_LABELS = {
  ...
  ownershipDate: {
    en: 'Ownership Date (Hijri)',
    ar: 'تاريخ الملكية (هجري)',
    hi: 'स्वामित्व तिथि (हिजरी)',
    bn: 'মালিকানা তারিখ (হিজরি)',
    ur: 'ملکیت کی تاریخ (ہجری)',
  },
  licenseIssuanceDate: {
    en: 'License Issuance Date (Hijri)',
    ar: 'تاريخ إصدار الرخصة (هجري)',
    hi: 'लाइसेंस जारी तिथि (हिजरी)',
    bn: 'লাইসেন্স ইস্যু তারিখ (হিজরি)',
    ur: 'لائسنس اجراء تاریخ (ہجری)',
  },
  inspectionExpiryDate: {
    en: 'Inspection Expiry Date',
    ar: 'تاريخ انتهاء الفحص',
    hi: 'निरीक्षण समाप्ति तिथि',
    bn: 'ইন্সপেকশন মেয়াদ শেষের তারিখ',
    ur: 'معائنہ میعاد ختم تاریخ',
  },
  restrictionStatus: {
    en: 'Restriction Status',
    ar: 'حالة القيود',
    hi: 'प्रतिबंध स्थिति',
    bn: 'নিষেধাজ্ঞার অবস্থা',
    ur: 'پابندی کی حالت',
  },
} as const;
...
export function VehicleFormScreen({
  ...
  const failedToLoadVehicleMessage = i18n.failedToLoadVehicle;
  const L = (k: keyof typeof EXTRA_LABELS) =>
    EXTRA_LABELS[k][locale] ?? EXTRA_LABELS[k].en;
  const alignedInputStyle = rtl ? styles.inputRtl : styles.inputLtr;
  const headerRowDirectionStyle = rtl
    ? styles.headerRowRtl
    : styles.headerRowLtr;
  const statusPillRowStyle = rtl
    ? styles.statusPillRowRtl
    : styles.statusPillRowLtr;
  ...
  const [hijriDatePickerField, setHijriDatePickerField] = useState<
    'ownershipDate' | 'licenseIssuanceDate' | 'licenseExpiryDate' | null
  >(null);
  ...
  const filteredDrivers = drivers.filter(
    (d) =>
      !driverQuery.trim() ||
      d.fullName.toLowerCase().includes(driverQuery.toLowerCase())
  );
  ...
  const pickerPlaceholderStyle = !selectedDriver
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([v, d]) => {
        ...
        const resolvedAssignedDriverId =
          v.assignedDriverId ?? v.drivers?.[0]?.id ?? '';
        const driverName =
          d.find((dr: DriverOption) => dr.id === resolvedAssignedDriverId)
            ?.fullName ??
  ...
  }, [failedToLoadVehicleMessage, isEdit, vehicleId]);
  ...
  async function handleSubmit() {
    ...
    const payload: Record<string, unknown> = {
      ...
      ...(form.operationCardIssueDate && {
        operationCardIssueDate: form.operationCardIssueDate,
      }),
      ...(form.operationCardExpiryDate && {
        operationCardExpiryDate: form.operationCardExpiryDate,
      }),
      ...(form.operationCardRenewDate && {
        operationCardRenewDate: form.operationCardRenewDate,
      }),
      ...(form.ownershipDate && { ownershipDate: form.ownershipDate }),
      ...(form.licenseIssuanceDate && {
        licenseIssuanceDate: form.licenseIssuanceDate,
      }),
      ...(form.inspectionExpiryDate && {
      ...
      ...(form.restrictionStatus.trim() && {
        restrictionStatus: form.restrictionStatus.trim(),
      }),
  ...
  return (
    <View style={styles.container}>
          ...
          <View style={styles.card}>
            ...
            <FormField label={i18n.issueDate}>
              <DateInputRow
                ...
                onOpenPicker={() =>
                  setDatePickerField('operationCardIssueDate')
                }
                rtl={rtl}
              />
            </FormField>
            ...
            <FormField label={i18n.expiryDateLabel}>
              <DateInputRow
                ...
                onOpenPicker={() =>
                  setDatePickerField('operationCardExpiryDate')
                }
                rtl={rtl}
              />
            </FormField>
            ...
            <FormField label={i18n.renewDate}>
              <DateInputRow
                ...
                onOpenPicker={() =>
                  setDatePickerField('operationCardRenewDate')
                }
                rtl={rtl}
              />
            </FormField>
          ...
          <View style={styles.card}>
            ...
            <FormField label={L('licenseIssuanceDate')}>
              <DateInputRow
                ...
                placeholder="1446-01-01"
                onOpenPicker={() =>
                  setHijriDatePickerField('licenseIssuanceDate')
                }
                rtl={rtl}
              />
            </FormField>
            ...
            <FormField label={i18n.licenseExpiryVehicle}>
              <DateInputRow
                ...
                placeholder="1446-01-01"
                onOpenPicker={() =>
                  setHijriDatePickerField('licenseExpiryDate')
                }
                rtl={rtl}
              />
            </FormField>
              ...
              <View style={styles.card}>
                ...
                <FormField label={i18n.vehicleStatusLabel}>
                  <ScrollView
                    horizontal
                    showsHorizontalScrollIndicator={false}
                    style={styles.statusPillScroll}
                  >
                    <View style={statusPillRowStyle}>
                      {VEHICLE_STATUSES.map((s) => (
                        <TouchableOpacity
                          key={s}
                          style={[
                            styles.statusPill,
                            form.status === s && styles.statusPillActive,
                          ]}
                          onPress={() =>
                        ...
                        >
                          <Text
                            style={[
                              ...
                              form.status === s && styles.statusPillTextActive,
                            ]}
                          >
                ...
                <FormField label={i18n.assignedDriverLabel}>
                  <TouchableOpacity
                    ...
                    activeOpacity={0.75}
                  >
                    <Text style={[styles.pickerText, pickerPlaceholderStyle]}>
          ...
          <View style={styles.modalSheet}>
            ...
            <FlatList
              ...
              ListHeaderComponent={
                ...
                >
                  <Text
                    style={[styles.driverItemText, styles.driverItemTextMuted]}
                  >
                    {i18n.unassigned}
                  </Text>
                </TouchableOpacity>
              ...
              renderItem={({ item }) => (
                ...
                >
                  <Text
                    style={[
                      styles.driverItemText,
                      form.assignedDriverId === item.id &&
                        styles.driverItemTextActive,
                    ]}
                  >
                    {item.fullName}
                  </Text>
      ...
      <DateWheelModal
        ...
        doneLabel={i18n.done}
        label={
          datePickerField === 'operationCardIssueDate'
            ? i18n.issueDate
            : datePickerField === 'operationCardExpiryDate'
            ? i18n.expiryDateLabel
            : datePickerField === 'operationCardRenewDate'
            ? i18n.renewDate
            : datePickerField === 'inspectionExpiryDate'
            ? L('inspectionExpiryDate')
      ...
      <DateWheelModal
        ...
        doneLabel={i18n.done}
        label={
          hijriDatePickerField === 'ownershipDate'
            ? L('ownershipDate')
            : hijriDatePickerField === 'licenseIssuanceDate'
            ? L('licenseIssuanceDate')
        ...
        onConfirm={(date) => {
          if (hijriDatePickerField) {
            set(hijriDatePickerField, date);
          }
...
function DateInputRow({
  ...
  const dateInputRowDirectionStyle = rtl
    ? styles.dateInputRowRtl
    : styles.dateInputRowLtr;
  const dateFieldAlignmentStyle = rtl
    ? styles.dateFieldTextRtl
    : styles.dateFieldTextLtr;

  return (
    <TouchableOpacity
      onPress={onOpenPicker}
      style={[styles.dateInputRow, dateInputRowDirectionStyle]}
      activeOpacity={0.75}
    >
      <Text
        style={[
          styles.dateFieldText,
          dateFieldAlignmentStyle,
...
const styles = StyleSheet.create({
  ...
  statusPillScroll: { marginTop: 6 },
  statusPillRowLtr: { flexDirection: 'row-reverse', gap: 8, paddingBottom: 4 },
  statusPillRowRtl: { flexDirection: 'row', gap: 8, paddingBottom: 4 },
  ...
  pickerTextPlaceholder: { color: Colors.textMuted },
  ...
  driverItemTextMuted: { color: Colors.textMuted },
  driverItemTextActive: { color: Colors.primary, fontWeight: '700' as const },
  driverEmpty: {
    textAlign: 'center',
    color: Colors.textMuted,
    paddingVertical: 24,
